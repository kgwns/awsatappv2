...
describe('<Author Item>', () => {
    ...
    beforeEach(() => {
        ...
        (fetchNarratedOpinionArticleApi as jest.Mock).mockImplementation(fetchNarratedOpinionArticleApiMock);
        (fetchNarratedOpinionArticleApiMock).mockReturnValue({playList:{duration:'duration'}});
        (isDarkTheme as jest.Mock).mockReturnValue(true);
        (useState as jest.Mock).mockImplementation(() => [{
            playlist: [
                {
                    name: 'abc',
    ...
    it("should display summary color in dark mode",() => {
        const testId = instance.getByTestId('listenToArticleId');
        expect(testId.props.style[0].color).toEqual(Styles.color.doveGray)
    });
});
...
describe('<Author Item>', () => {
    ...
    beforeEach(() => {
        ...
        (fetchNarratedOpinionArticleApi as jest.Mock).mockImplementation(fetchNarratedOpinionArticleApiMock);
        (fetchNarratedOpinionArticleApiMock).mockReturnValue({playList:{duration:'duration'}});
        (isDarkTheme as jest.Mock).mockReturnValue(false);
        (useState as jest.Mock).mockImplementation(() => [{
            playlist: [
                {
                    name: 'abc',
    ...
    it("should display green600 color in light mode",() => {
        const testId = instance.getByTestId('listenToArticleId');
        expect(testId.props.style[0].color).toEqual(Styles.color.green600)
    });
})
