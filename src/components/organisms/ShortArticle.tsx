...
const ShortArticle = ({ data, headerLeft,onPress }: ArticleSectionProps) => {
  const renderItem = (item: ShortArticleProps, index: number) => {
    shortArticleFooter.rightTitle = item.author
    ...
    return <TouchableWithoutFeedback onPress={() => onPress(item.nid)}>
      <View key={flatListUniqueKey.SHORT_ARTICLE + index} style={{ paddingBottom: normalize(20) }}>
        <View style={{ flexDirection: 'row' }}>
          ...
          <Image url={item.image} style={ShortArticleStyle.image} resizeMode={ImageResize.COVER} />
        </View>
        {(isLasIndex) && <Divider />}
      </View>
